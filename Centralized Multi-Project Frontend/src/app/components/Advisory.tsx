...
import { AlertTriangle, CheckCircle, Send, Bot, User, Sparkles, Database, ShieldCheck, Trash2, Activity, HelpCircle, ShoppingCart } from "lucide-react";
import { useLocation, useNavigate } from "react-router-dom";
import { transferAPI, requestsAPI, inventoryAPI, sitesAPI, advisoryAPI } from "../../services/apiService";
...
export function Advisory() {
  ...
  const [userSiteId, setUserSiteId] = useState<number>(1);

  // --- HOISTED STATE FIX: Retrieve from sessionStorage so it survives navigation ---
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    const savedChat = sessionStorage.getItem("mattrack_chat_history");
  ...
  const [inputMessage, setInputMessage] = useState(() => sessionStorage.getItem("mattrack_chat_draft") || "");
  const [isTyping, setIsTyping] = useState(false);
  const chatEndRef = useRef<HTMLDivElement>(null);

  // --- SAVE STATE ON CHANGE ---
  useEffect(() => { sessionStorage.setItem("mattrack_chat_history", JSON.stringify(messages)); }, [messages]);
  useEffect(() => { sessionStorage.setItem("mattrack_chat_draft", inputMessage); }, [inputMessage]);
  ...
  useEffect(() => {
    const fetchCriticalStock = async () => {
      setLoading(true);
      try {
        const [inventory, sites] = await Promise.all([ inventoryAPI.list(), sitesAPI.list() ]);
        const siteNames = sites.map((s: any) => s.site_name);
        ...
        if (token) {
          const payload = JSON.parse(atob(token.split(".")[1]));
          uid = payload.id;
          role = payload.role ? payload.role.toLowerCase() : "staff";
        }

        if (uid) {
          const mySite = sites.find((s: any) => s.manager_id === uid);
          if (mySite) {
            mySiteId = mySite.id;
            setUserSiteId(mySite.id);
          }
        }

        const siteMap = new Map(sites.map((s: any) => [s.id, s.site_name]));
        let urgent = inventory.filter((item: any) => item.status === "Critical" || item.status === "Low Stock");

        if (role === "staff") {
           urgent = urgent.filter((item: any) => item.site_id === mySiteId);
        }

        setCriticalItems(urgent.map((item: any) => ({ ...item, siteName: siteMap.get(item.site_id) || `Site ${item.site_id}` })));
      } catch (error) { console.error("Failed to fetch critical stock"); } finally { setLoading(false); }
    };
    fetchCriticalStock();
  }, [currentUserId]);
  ...
  const handleLogisticsAction = async (sourceId: string, itemName: string, brand: string, qty: string, unit: string) => {
    ...
    if (isAdmin) {
      const isConfirmed = window.confirm(`SECURITY VERIFICATION:\n\nAre you sure you want to dispatch ${qty} ${unit} of ${itemName} from Site ${sourceId} to your location?`);
      if (!isConfirmed) return;
      try {
        setIsTyping(true);
        await transferAPI.initiate({ source_site_id: Number(sourceId), destination_site_id: userSiteId, item_name: itemName.trim(), brand: brand.trim() || "Generic/No Brand", quantity: Number(qty), unit: unit.trim(), });
        setMessages((prev) => [...prev, { id: Date.now().toString(), role: "ai", content: `✅ Transfer initiated! Logistics network alerted to move ${qty} ${unit} of ${itemName} from Site ${sourceId}.` }]);
      } catch (error: any) { alert(error.message || "Failed to execute transfer command."); } finally { setIsTyping(false); }
    } else {
      ...
      try {
        setIsTyping(true);
        await requestsAPI.create({ item_name: itemName.trim(), brand: brand.trim() || "Generic/No Brand", quantity_needed: Number(qty), unit: unit.trim(), site_id: userSiteId, status: "Pending Approval" });
        setMessages((prev) => [...prev, { id: Date.now().toString(), role: "ai", content: `✅ Material Request successfully submitted! It is now in the Admin's Procurement Queue.` }]);
  ...
  const handleClearChat = () => {
    if (window.confirm("Are you sure you want to clear the context and session history?")) {
      ...
      setInputMessage("");
      sessionStorage.setItem("mattrack_chat_history", JSON.stringify(resetMessage));
    ...
    <div className="space-y-6 animate-in fade-in duration-500 h-[calc(100vh-8rem)] flex flex-col bg-slate-50 p-6">
      <div>
        <h1 className="text-2xl font-bold text-neutral-900 flex items-center gap-2">
          <Sparkles className="w-6 h-6 text-emerald-500" /> Procurement Advisor
        </h1>
        <p className="text-sm text-neutral-500 mt-1">Enterprise heuristic engine powered by real-time inventory telemetry.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
          ...
          <div className="p-4 overflow-y-auto flex-1">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center text-center h-full space-y-2 opacity-70">
                ...
                <p className="text-sm font-medium text-emerald-800">{["admin", "owner"].includes(currentUserRole) ? "All network sites are optimal." : "Your assigned site is optimal."}</p>
              </div>
            ) : (
                ...
                {criticalItems.map((item) => (
                  <div key={item.id} className="p-4 border border-slate-200 rounded-xl hover:border-emerald-300 transition-colors bg-white shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      <div><h3 className="font-bold text-slate-900">{item.item_name}</h3><p className="text-xs text-slate-500">{item.siteName}</p></div>
        ...
        <div className="lg:col-span-2 flex flex-col bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden relative">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-4 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === "ai" ? "bg-emerald-100 text-emerald-600 border border-emerald-200" : "bg-slate-800 text-white"}`}>
                  {msg.role === "ai" ? <Activity className="w-4 h-4" /> : <User className="w-4 h-4" />}
                </div>
                <div className={`max-w-[85%] rounded-2xl px-5 py-3.5 text-sm shadow-sm whitespace-pre-wrap ${msg.role === "user" ? "bg-slate-800 text-white rounded-tr-none" : "bg-white border border-slate-200 text-slate-700 rounded-tl-none leading-relaxed font-medium"}`}>
                  {(() => {
                    ...
                    if (match) {
                      ...
                      return (
                        <div className="flex flex-col">
                          ...
                          {msg.role === "ai" && (
                            <button onClick={() => handleLogisticsAction(siteId, itemName, brand, qty, unit)} className={`w-full mt-2 text-white font-bold py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 transition-all shadow-sm active:scale-95 ${["admin", "owner"].includes(currentUserRole) ? "bg-emerald-600 hover:bg-emerald-700" : "bg-amber-600 hover:bg-amber-700"}`}>
                              {["admin", "owner"].includes(currentUserRole) ? (<><Send className="w-4 h-4" /> Dispatch {qty} {unit}</>) : (<><ShoppingCart className="w-4 h-4" /> Request {qty} {unit}</>)}
                            </button>
                          )}
                        </div>
                      );
            ...
            {isTyping && (
              <div className="flex gap-4">
                <div className="w-8 h-8 rounded-full bg-emerald-100 text-emerald-600 border border-emerald-200 flex items-center justify-center shrink-0"><Bot className="w-4 h-4" /></div>
          ...
          <div className="px-4 py-2 bg-slate-100/80 border-t border-slate-200 flex items-center gap-2 overflow-x-auto">
            <span className="text-xs font-bold text-slate-500 shrink-0 flex items-center gap-1"><HelpCircle className="w-3.5 h-3.5 text-emerald-600" /> Suggested Prompts:</span>
            {FAQ_PROMPTS.map((promptText, i) => (
              <button key={i} onClick={() => handleSendMessage(undefined, promptText)} disabled={isTyping} className="px-3 py-1 bg-white border border-slate-200 rounded-full text-xs font-medium text-slate-700 hover:border-emerald-500 hover:text-emerald-700 transition-colors whitespace-nowrap shrink-0 shadow-2xs">
                {promptText}
              </button>
            ))}
          </div>
