...
import {
  AlertTriangle, CheckCircle, Send, Bot, User, Sparkles, Database,
  ShieldCheck, Trash2, Activity, HelpCircle, ShoppingCart
} from "lucide-react";
...
type Inventory = { id: number; item_name: string; brand?: string; quantity: number; unit: string; status: string; site_id: number; };

const inventoryAPI = { list: () => fetchAPI<Inventory[]>("/inventory/") };
const sitesAPI = { list: () => fetchAPI<any[]>("/sites/") };
const advisoryAPI = { askAI: (message: string) => fetchAPI<{ reply: string }>("/advisory/chat", { method: "POST", body: JSON.stringify({ message }) }) };
...
export function Advisory() {
  ...
  const storageKey = `mattrack_ai_chat_${currentUserId || "default"}`;
  ...
  useEffect(() => {
    const fetchCriticalStock = async () => {
      ...
      try {
        ...
        if (uid) {
          const mySite = sites.find(s => s.manager_id === uid);
          if (mySite) {
            mySiteId = mySite.id;
        ...
        setCriticalItems(urgent.map((item) => ({ ...item, siteName: siteMap.get(item.site_id) || `Site ${item.site_id}` })));
  ...
  const handleLogisticsAction = async (sourceId: string, itemName: string, brand: string, qty: string, unit: string) => {
    ...
    if (isAdmin) {
      ...
      if (!isConfirmed) {
        setMessages((prev) => [...prev, { id: Date.now().toString(), role: "ai", content: `❌ Transfer request aborted by Admin.` }]);
        return;
      }
      try {
        setIsTyping(true);
        await transferAPI.initiate({
          source_site_id: Number(sourceId), destination_site_id: userSiteId, item_name: itemName.trim(),
      ...
      if (!isConfirmed) {
        setMessages((prev) => [...prev, { id: Date.now().toString(), role: "ai", content: `❌ Material request aborted by user.` }]);
        return;
      }
      try {
        setIsTyping(true);
        await requestsAPI.create({
          item_name: itemName.trim(), brand: brand.trim() || "Generic/No Brand", quantity_needed: Number(qty),
          unit: unit.trim(), site_id: userSiteId, status: "Pending Approval"
        });
        setMessages((prev) => [...prev, { id: Date.now().toString(), role: "ai", content: `✅ Material Request successfully submitted! It is now in the Admin's Procurement Queue pending final approval.` }]);
                        ...
                        <div className="flex flex-col">
                          ...
                          {msg.role === "ai" && (
                            <button onClick={() => handleLogisticsAction(siteId, itemName, brand, qty, unit)} className={`w-full mt-2 text-white font-bold py-2.5 px-4 rounded-lg flex items-center justify-center gap-2 transition-all shadow-sm active:scale-95 ${["admin", "owner"].includes(currentUserRole) ? "bg-emerald-600 hover:bg-emerald-700" : "bg-amber-600 hover:bg-amber-700"}`}>
                              {["admin", "owner"].includes(currentUserRole) ? (<><Send className="w-4 h-4" /> Dispatch {qty} {unit} from Site {siteId}</>) : (<><ShoppingCart className="w-4 h-4" /> Request {qty} {unit} via Admin</>)}
                            </button>
                          )}
                        </div>
                      );
...
export default function App() { return <Advisory />; }