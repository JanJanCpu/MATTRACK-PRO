...
interface InventoryWithCategory extends InventoryItem {
  ...
  siteName: string;
  baseline_quantity: number; // Added to handle dynamic progress bar
}
...
export function Inventory() {
  ...
  const [activeTab, setActiveTab] = useState<
    "inventory" | "audit" | "incoming" | "procurement"
  >("inventory");
  ...
  // This state no longer has "status" manually inputted in the form, 
  // but we keep a default in state to send to the backend.
  ...
  const handleSmartCatalogSelect = (selectedName: string) => {
    ...
    if (match) {
      ...
      setNewItem(prev => ({ ...prev, item_name: selectedName }));
    }
  };
  ...
  useEffect(() => {
    ...
    if (location.state?.autoPivotSiteId) {
      ...
      setNewItem((prev) => ({
        ...prev,
        site_id: String(location.state.autoPivotSiteId),
      }));
    }
  ...
  // Here is where the default status is automatically set silently based on the type
  useEffect(() => {
    if (itemType === "consumable") {
      setNewItem((prev) => ({ ...prev, unit: "Bags", status: "In Stock" }));
    } else {
  ...
  const fetchData = async () => {
    try {
      ...
      if (token) {
        try {
            const posRes = await fetch(`http://${window.location.hostname}:8000/inventory/purchase-orders`, {
                headers: { "Authorization": `Bearer ${token}` }
            });
            if (posRes.ok) pos = await posRes.json();
        } catch (e) {
  ...
  const handleAcceptTransfer = async (transferId: number) => {
    if (
      !window.confirm(
        "SECURITY VERIFICATION: Confirm receipt of these materials? They will be securely merged into your site's physical ledger.",
      )
    )
      return;
  ...
  const handleReceivePO = async (poId: number) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://${window.location.hostname}:8000/inventory/purchase-orders/${poId}/receive`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
        alert("✅ External PO Shipment Received! Physical inventory count directly updated.");
        fetchData();
      } else {
        alert("Failed to mark shipment as received.");
      }
    } catch (err) {
      alert("Network error. Could not connect to API.");
    }
  ...
  const handleCancelPO = async (poId: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://${window.location.hostname}:8000/inventory/purchase-orders/${poId}/cancel`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleAddInventory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Failed to register baseline item to the ledger.");
    }
  };
  ...
  const handleTransactionSubmit = async (e: React.FormEvent) => {
    ...
    let finalStatus = activeTransactionItem.status; 
  ...
  const exportToCSV = () => {
    ...
    const rows = sortedData.map(
      (item) => `"${item.item_name}","${item.brand}","${item.siteName}",${item.quantity},${item.baseline_quantity},"${item.unit}","${item.status}"`,
    );
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-neutral-200 pb-4">
        <div>
          ...
          <div className="flex bg-neutral-100 p-1 rounded-lg w-fit mt-4 flex-wrap gap-1">
            <button
            ...
            >
              <FilePlus className="w-4 h-4" /> Procurement Queue
            </button>
          </div>
        </div>
        ...
        {activeTab === "inventory" && (
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center bg-white border border-neutral-300 rounded-lg px-3 py-1.5 shadow-2xs">
            ...
            <div className="bg-white p-6 rounded-xl border border-emerald-200 shadow-sm animate-in slide-in-from-top-4">
              ...
              >
                <div>
                  ...
                  <select
                    className="w-full p-2 border rounded-lg text-sm bg-white focus:ring-2 focus:ring-emerald-500 outline-none font-medium"
                ...
                <div>
                  <label className="block text-xs font-bold text-neutral-500 mb-1">
                    {itemType === "consumable" ? "Master Item Name" : "Tool Name"}
                  </label>
                  <input
                    type="text"
                    list="catalog-items"
                    className="w-full p-2 border rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none font-medium"
                    placeholder="Search master catalog..."
                    value={newItem.item_name}
                    onChange={(e) => handleSmartCatalogSelect(e.target.value)}
                    required
                  />
                  <datalist id="catalog-items">
                    {MASTER_CATALOG.map(c => (
                      <option key={c.sku} value={c.name}>{c.sku}: {c.brand}</option>
                    ))}
                  </datalist>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      type="number"
                      placeholder="0"
                      className="w-full p-2 border rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none font-bold"
                ...
                <button
                  type="submit"
                  className="bg-slate-900 text-white py-2 rounded-lg font-bold hover:bg-slate-800 transition-colors w-full h-full max-h-[38px] flex items-center justify-center"
                >
                  Register Baseline
                </button>
              </form>
                  ...
                  {sortedData.length > 0 ? (
                            ...
                            <div className="flex flex-col items-end gap-1">
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`text-base font-bold ${item.status === 'Depleted' ? 'text-neutral-400 line-through' : 'text-neutral-900'}`}>
                                  ...
                                  <span className="text-neutral-400 font-normal text-xs">
                                    / {item.baseline_quantity} {item.unit}
                                  </span>
                                </span>
                              </div>
                              ...
                              {!isAsset && (
                                <div className="w-full bg-neutral-200 rounded-full h-1.5 mt-1 overflow-hidden">
                                  <div 
                                    className={`h-1.5 rounded-full ${progressPercent <= 10 ? 'bg-red-500' : progressPercent <= 30 ? 'bg-amber-500' : 'bg-emerald-500'}`} 
                              ...
                              {canEdit ? (
                                <>
                                  <button
                                    ...
                                    title={isAsset ? "Mark as Available" : "Restock Delivery Intake"}
                                  >
                                  ...
                                  <button
                                    ...
                                    title={isAsset ? "Mark as In Use" : "Log Field Consumption"}
                                  >
                                  ...
                                  {isCriticalOrLow ? (
                                    <button
                                      ...
                                      title="Cannot dispatch emergency stock (Critical/Low Stock)"
                                    >
                                    ...
                                    <button
                                      ...
                                      title="Dispatch Site Transfer"
                                    >
                                      <Send className="w-4 h-4" />
                                    </button>
                                  )}

                                  {/* --- PM Lifecycle Override Button --- */}
                                  {!isAsset && (
                                    <button
                                      onClick={() => {
                                        setActiveTransactionItem(item);
                                      ...
                                      className="p-2 ml-1 text-indigo-600 hover:bg-indigo-50 border border-transparent hover:border-indigo-200 rounded-lg transition-colors"
                                      title="Manage Status Lifecycle (Sufficient, Surplus, Depleted)"
                                    >
                    ...
                    <tr>
                      <td
                        ...
                        className="px-5 py-12 text-center text-neutral-500 bg-neutral-50/50 font-medium"
                      >
                        No inventory matches your current scope.
                      </td>
                    </tr>
                  )}
                </tbody>
              ...
              <tbody className="divide-y divide-neutral-100">
                {incomingTransfers.length > 0 ? (
                  incomingTransfers.map((transfer) => {
                    const sourceSite = sitesList.find(
                      (s) => s.id === transfer.source_site_id,
                    );
                    const destSite = sitesList.find(
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-4 text-neutral-500 font-mono text-xs font-bold">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex flex-col gap-1 text-xs">
                            <span className="flex items-center gap-1 font-medium text-slate-700">
                              <Building2 className="w-3 h-3 text-slate-400" /> From: {sourceSite ? sourceSite.site_name : `Site ${transfer.source_site_id}`}
                            </span>
                            <span className="flex items-center gap-1 font-bold text-indigo-700">
                              <MapPin className="w-3 h-3 text-indigo-400" /> To: {destSite ? destSite.site_name : `Site ${transfer.destination_site_id}`}
                            </span>
                          </div>
                        </td>
                        <td className="px-5 py-4">
                        ...
                        <td className="px-5 py-4 text-right">
                          <div className="flex justify-end gap-2">
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-red-200 hover:bg-red-50 text-red-600 rounded-lg text-xs font-bold transition-colors shadow-sm"
                            >
          ...
          <div className="px-6 py-5 border-b border-neutral-200 bg-blue-50 flex items-center justify-between">
            <div>
              <h2 className="font-semibold text-blue-900 flex items-center gap-2">
                <FilePlus className="w-5 h-5" /> Procurement Queue
              </h2>
                ...
                {pendingPOs.filter((po: any) => po.status !== "Received" && po.status !== "Cancelled").length > 0 ? (
                      ...
                      return (
                        <tr key={po.id} className="hover:bg-neutral-50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-neutral-600 font-medium">
                            {sup ? sup.name : "Unknown Vendor"}
                          </td>
                          <td className="px-6 py-4 text-center font-bold text-emerald-700">{po.quantity}</td>
                          <td className="px-6 py-4 text-center">
                            <span className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${
                                po.status === "Pending" ? "bg-amber-100 text-amber-700 border border-amber-200" :
                                po.status === "Shipped" || po.status === "In-Transit" ? "bg-blue-100 text-blue-700 border border-blue-200" :
                            ...
                            <div className="flex items-center justify-end gap-2">
                              {po.status === "Pending" && (
                                <button
                                  ...
                                  className="px-3 py-1.5 border border-red-200 text-red-600 hover:bg-red-50 rounded-lg transition-colors text-xs font-bold inline-flex items-center gap-1.5 shadow-sm"
                                >
                              ...
                              <button
                                ...
                                className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors text-xs font-bold inline-flex items-center gap-1.5 shadow-sm"
                              >
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-neutral-400">
                      No pending vendor orders in the procurement queue.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="p-0">
            {auditLogs.length > 0 ? (
              ...
              <div className="py-20 text-center">
                ...
                <p className="text-sm text-neutral-500 mt-1">
                  Actions performed on the database will appear here chronologically.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-4 border-b flex justify-between items-center bg-indigo-50 text-indigo-900">
              <h3 className="font-bold flex items-center gap-2 text-lg">
                <Flag className="w-5 h-5 text-indigo-600" /> Manage Status Lifecycle
              </h3>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-3 pt-2">
                <button 
                  onClick={() => handleOverrideStatus("Sufficient")}
                  className="w-full text-left p-4 rounded-xl border border-emerald-200 bg-white hover:bg-emerald-50 transition-colors group"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 group-hover:scale-110 transition-transform"><CheckCircle className="w-4 h-4"/></div>
                    <div>
                      <h4 className="font-bold text-emerald-900">Flag as "Sufficient" (In Stock)</h4>
                      <p className="text-xs text-neutral-500 mt-0.5">The remaining 10% is enough to finish the project. Clear low stock warnings.</p>
                    </div>
                  </div>
                </button>

                <button 
                  onClick={() => handleOverrideStatus("Surplus")}
                  className="w-full text-left p-4 rounded-xl border border-blue-200 bg-white hover:bg-blue-50 transition-colors group"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 group-hover:scale-110 transition-transform"><PackageSearch className="w-4 h-4"/></div>
                    <div>
                      <h4 className="font-bold text-blue-900">Flag as "Surplus"</h4>
                      <p className="text-xs text-neutral-500 mt-0.5">We ordered more than we needed. Flag these items for potential network transfer.</p>
                    </div>
                  </div>
                </button>

                <button 
                  onClick={() => handleOverrideStatus("Depleted")}
                  className="w-full text-left p-4 rounded-xl border border-neutral-300 bg-white hover:bg-neutral-100 transition-colors group"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-neutral-200 flex items-center justify-center text-neutral-600 group-hover:scale-110 transition-transform"><Ban className="w-4 h-4"/></div>
                    <div>
                      <h4 className="font-bold text-neutral-900">Flag as "Depleted" (Used Up)</h4>
                      <p className="text-xs text-neutral-500 mt-0.5">Material is completely used up and no longer needed for this phase. Finalize lifecycle.</p>
                    </div>
                  </div>
                </button>
              </div>
      ...
      {(modalType === "IN" || modalType === "OUT" || modalType === "TRANSFER") &&
        activeTransactionItem &&
        createPortal(
          <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4 animate-in fade-in">
            <div
              className={`bg-white rounded-xl shadow-2xl w-full overflow-hidden flex flex-col md:flex-row transition-all duration-500 ease-in-out max-h-[90vh] ${modalType === "TRANSFER" ? "max-w-5xl" : "max-w-md"}`}
            >
              <div
              ...
              >
                ...
                >
                  <h2 className="text-lg font-bold flex items-center gap-2">
                    {modalType === "IN" && (
                      <>
                        <ArrowDownToLine className="w-5 h-5" /> Log Delivery Intake
                      </>
                    )}
                    {modalType === "OUT" && (
                      <>
                        <ArrowUpFromLine className="w-5 h-5" /> Log Field Usage
                      </>
                    )}
                    {modalType === "TRANSFER" && (
                      <>
                        <Send className="w-5 h-5" /> Dispatch Site Transfer
                      </>
                    )}
                  </h2>
                  <button
                ...
                >
                  <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-200">
                    ...
                    <p className="text-xs text-neutral-500 mt-1 font-medium">
                      Current Stock: {activeTransactionItem.quantity} / {activeTransactionItem.baseline_quantity} {activeTransactionItem.unit}
                    </p>
                  </div>
                  ...
                  {modalType === "TRANSFER" && (
                    <div>
                      ...
                      <select
                        ...
                        className="w-full p-3 border border-blue-200 bg-blue-50 rounded-lg text-sm font-bold text-neutral-900 focus:ring-2 focus:ring-blue-500 outline-none"
                      >
                        ...
                        {sitesList
                          ...
                          .map((site) => (
                            <option key={site.id} value={site.id}>
                              {site.site_name} — {(site as any).address || "Metro Manila"} (ID: SITE-{site.id})
                            </option>
                          ))}
                      </select>
                    </div>
                  ...
                  <div className="pt-2">
                    ...
                    >
                      {modalType === "TRANSFER"
                        ? "Confirm Dispatch"
                        : `Confirm ${modalType === "IN" ? "Delivery Intake" : "Field Consumption"}`}
                    </button>
                  </div>
                </form>
              </div>
              ...
              {modalType === "TRANSFER" && (
                <div className="hidden md:flex flex-col md:w-2/3 bg-slate-100 relative min-h-[500px]">
                  {transactionForm.destination_site_id ? (
                        ...
                        <>
                          ...
                          <div className="absolute top-4 left-4 z-[999] bg-white/95 backdrop-blur shadow-xl border border-neutral-200 rounded-xl p-5 max-w-sm pointer-events-none">
                            <h3 className="text-blue-600 font-bold text-xs tracking-wider uppercase flex items-center gap-2 mb-3">
                              <Navigation className="w-4 h-4" /> Live Route Telemetry
                            </h3>
                            <div className="flex items-start gap-3 mb-4">
                              ...
                              <div>
                                <p className="text-[10px] text-neutral-500 font-bold uppercase">Origin</p>
                                <p className="text-sm font-bold text-neutral-900 mb-1">{sourceSite.site_name}</p>
                                <p className="text-[10px] text-neutral-500 font-bold uppercase mt-2">Destination</p>
                                <p className="text-sm font-bold text-neutral-900">{destSite.site_name}</p>
                              </div>
                            </div>
                            <div className="grid grid-cols-2 gap-3 pt-3 border-t border-neutral-100">
                              <div>
                                ...
                                <p className="text-xl font-black text-neutral-800">
                                  ...
                                  <span className="text-xs font-medium text-neutral-500">km</span>
                                </p>
                              </div>
                              <div>
                                <p className="text-[10px] text-neutral-500 font-bold uppercase flex items-center gap-1">
                                ...
                                <p className="text-xl font-black text-blue-600">
                                  ~{routeDetails.eta}{" "}
                                  <span className="text-xs font-medium text-blue-400">mins</span>
                                </p>
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-neutral-400 font-medium">
                      <MapPin className="w-16 h-16 mb-4 opacity-20" />
                      <p className="text-sm">Select a destination to visualize the logistics route.</p>
                    </div>
                  )}
                </div>
              )}
