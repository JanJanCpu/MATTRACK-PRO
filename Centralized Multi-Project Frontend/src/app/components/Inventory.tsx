...
// Predefined Master SKU Catalog to eliminate typo and case-sensitivity issues
...
export function Inventory() {
  ...
  const [transactionForm, setTransactionForm] = useState<{
    ...
    supplier_id: string;
    batch_rating: number;
  }>({ quantity: "", destination_site_id: "", supplier_id: "", batch_rating: 0 });

  // Map Routing State
  const [transferRouteCoords, setTransferRouteCoords] = useState<[number, number][]>([]);
  const [routeDetails, setRouteDetails] = useState({ distance: "0", eta: "0" });

  // --- NEW ERP RESTOCK WORKFLOW ---
  const [showRequestModal, setShowRequestModal] = useState(false);
  const [requestItem, setRequestItem] = useState<InventoryWithCategory | null>(null);
  ...
  const [newItem, setNewItem] = useState<{
    ...
    item_name: "", brand: "Generic/No Brand", quantity: "", unit: "Bags",
    status: "In Stock", fsn_status: "FAST", site_id: "",
  });

  const [showRestockModal, setShowRestockModal] = useState(false);
  const [restockItem, setRestockItem] = useState<InventoryWithCategory | null>(null);
  ...
  // --- NEW ERP PROCUREMENT RECEIVING STATE ---
  const [receivingPO, setReceivingPO] = useState<any | null>(null);
  const [poRating, setPoRating] = useState<number>(0);

  const handleSmartCatalogSelect = (selectedName: string) => {
    const match = MASTER_CATALOG.find(c => c.name === selectedName);
    if (match) setNewItem(prev => ({ ...prev, item_name: match.name, brand: match.brand, unit: match.unit }));
    else setNewItem(prev => ({ ...prev, item_name: selectedName }));
  };

  // ERP: Submit 1-Click Request to Admin
  const handleRequestRestock = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!requestItem || !requestQty) return;
    try {
      await requestsAPI.restock(requestItem.id, requestQty);
      alert("✅ Restock request submitted! It is now pending Admin approval.");
  ...
  // ADMIN: Execute fulfillment from Advisor
  const handleExecuteRestock = async (option: any) => {
    ...
    try {
      if (option.type === "EXTERNAL_PURCHASE") {
        ...
        alert(`Logistics network alerted. Transfer from ${option.source_name} is now IN TRANSIT.`);
      }
        ...
        const fetchRoute = async () => {
          try {
            ...
            if (data.routes && data.routes.length > 0) {
              ...
              const durationMins = Math.max(10, Math.round(data.routes[0].duration / 60)).toString();
              setRouteDetails({ distance: distKm, eta: durationMins });
            }
          } catch (error) {
            setTransferRouteCoords([[sourceSite.latitude, sourceSite.longitude], [destSite.latitude, destSite.longitude]]);
          }
        };
        fetchRoute();
      }
  ...
  useEffect(() => {
    ...
    if (location.state?.autoPivotSiteId) {
      ...
      setNewItem((prev) => ({ ...prev, site_id: String(location.state.autoPivotSiteId) }));
    }
  ...
  useEffect(() => {
    if (itemType === "consumable") setNewItem((prev) => ({ ...prev, unit: "Bags", status: "In Stock" }));
    else setNewItem((prev) => ({ ...prev, unit: "Unit", status: "Available" }));
  }, [itemType]);
  ...
  const fetchData = async () => {
    try {
      ...
      let pos = [];
      try { pos = await purchaseOrdersAPI.list(); } catch (e) {}
      ...
      const categorized = inventoryList.map((item) => ({
        ...item,
        category: (item.status === "Critical" || item.status === "Low Stock") ? "Fast-Moving"
            : (item.status === "In Stock" || item.status === "Available" || item.status === "Sufficient") ? "Slow-Moving" : "Non-Moving",
  ...
  const loadIncomingTransfers = async () => {
    try {
      const managedSites = sitesList.filter((s) => currentUserRole !== "staff" || s.manager_id === currentUserId);
      let allPending: any[] = [];
  ...
  useEffect(() => {
    if (activeTab === "incoming") loadIncomingTransfers();
  }, [activeTab, sitesList]);

  const handleAcceptTransfer = async (transferId: number) => {
    if (!window.confirm("SECURITY VERIFICATION: Confirm receipt of these materials?")) return;
    try {
      await transferAPI.receive(transferId);
      alert("✅ Transfer successfully received! Critical statuses automatically updated.");
  ...
  const handleCancelTransfer = async (transferId: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://${window.location.hostname}:8000/transfers/${transferId}/cancel`, {
        method: "POST",
      ...
      if (res.ok) {
        ...
        fetchData();
        window.dispatchEvent(new Event("inventoryUpdated"));
      } else { alert("Failed to reject transfer."); }
    } catch (err) { alert("Network error."); }
  };

  const submitReceivePO = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!receivingPO) return;
    if (poRating === 0) return alert("Please provide a delivery rating before confirming.");

    try {
      await purchaseOrdersAPI.receive(receivingPO.id, poRating);
      alert("✅ External PO Shipment Received! Supplier rating recorded.");
      setReceivingPO(null);
      setPoRating(0);
      fetchData();
      window.dispatchEvent(new Event("inventoryUpdated"));
    } catch (err: any) { 
      alert(err.message || "Network error. Could not mark shipment as received."); 
    }
  };

  const handleCancelPO = async (poId: number) => {
    if (!window.confirm("Are you sure you want to cancel this pending purchase order?")) return;
    try {
      await purchaseOrdersAPI.cancel(poId);
      alert("❌ Purchase Order successfully cancelled.");
      fetchData();
    } catch (err: any) { alert(err.message || "Network error. Could not cancel PO."); }
  };

  const handleAddInventory = async (e: React.FormEvent) => {
    e.preventDefault();
    if (newItem.quantity === "") return alert("Please specify physical item count.");
    try {
      ...
      setNewItem({ item_name: "", brand: "Generic/No Brand", quantity: "", unit: "Bags", status: "In Stock", fsn_status: "FAST", site_id: siteFilter ? String(siteFilter) : "" });
      fetchData();
      window.dispatchEvent(new Event("inventoryUpdated"));
    } catch (err) { alert("Failed to register baseline item to the ledger."); }
  };

  const handleOverrideStatus = async (status: string) => {
    if (!activeTransactionItem) return;
    try {
      await inventoryAPI.overrideStatus(activeTransactionItem.id, status);
      setModalType(null);
      fetchData();
      window.dispatchEvent(new Event("inventoryUpdated"));
  ...
  const handleTransactionSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!activeTransactionItem || !modalType || transactionForm.quantity === "") return;
    if (Number(transactionForm.quantity) <= 0) return alert("Quantity must be greater than 0");

    if (modalType === "TRANSFER") {
      if (!transactionForm.destination_site_id) return alert("Select a destination site.");
      if (activeTransactionItem.quantity < Number(transactionForm.quantity)) return alert("Not enough stock to transfer.");
      ...
      } catch (err: any) { alert(err.message || "Failed to initiate transfer."); }
      return;
    }

    let finalQuantity = Number(transactionForm.quantity);
    if (modalType === "OUT") {
      if (activeTransactionItem.quantity < Number(transactionForm.quantity))
        return alert(`Not enough stock! You only have ${activeTransactionItem.quantity} ${activeTransactionItem.unit}.`);
      finalQuantity = -Math.abs(Number(transactionForm.quantity));
    }
    ...
    try {
      await inventoryAPI.logTransaction({
        ...
        unit: activeTransactionItem.unit,
        status: activeTransactionItem.status,
        fsn_status: "FAST",
        site_id: activeTransactionItem.site_id,
        supplier_id: undefined, // Cleared because Procurement handles this now
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      fetchData();
      window.dispatchEvent(new Event("inventoryUpdated"));
    } catch (err) { alert("An error occurred. You may not have clearance for all selected sites."); fetchData(); }
  };

  const exportToCSV = () => {
    if (inventoryData.length === 0) return alert("No data to export.");
    const headers = ["Item Name", "Brand", "Location", "Quantity", "Baseline 100%", "Unit", "Status"];
    const rows = sortedData.map((item) => `"${item.item_name}","${item.brand}","${item.siteName}",${item.quantity},${item.baseline_quantity},"${item.unit}","${item.status}"`);
  ...
  if (siteFilter) processedData = processedData.filter((i) => i.site_id === siteFilter);
  if (statusFilter !== "All") processedData = processedData.filter((i) => i.status === statusFilter);

  const getStatusWeight = (status: string) => {
    switch (status) {
      case "Critical": case "Maintenance": case "Depleted": return 1;
      case "Low Stock": return 2;
      case "In Stock": case "Sufficient": case "Available": return 3;
    ...
    if (e.target.checked) {
      const editableIds = sortedData
        .filter((item: InventoryWithCategory) => {
          const site = sitesList.find((s: ProjectSite) => s.id === item.site_id);
          return currentUserRole !== "staff" || (site && site.manager_id === currentUserId);
        })
        .map((item: InventoryWithCategory) => item.id);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "In Stock": case "Available": case "Sufficient": return "bg-emerald-100 text-emerald-700 border-emerald-200";
      ...
      case "Depleted": return "bg-neutral-100 text-neutral-500 border-neutral-300";
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-4 border-b border-neutral-200 pb-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">
            Global visibility enabled. Select your active project site below to isolate ledgers.
          </p>
        </div>

        {/* The Tabs: Now isolated on their own row, fully protected from squeezing */}
        <div className="flex bg-neutral-100 p-1 rounded-lg w-max max-w-full overflow-x-auto gap-1">
          <button 
            onClick={() => setActiveTab("inventory")} 
            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-bold transition-all whitespace-nowrap shrink-0 ${activeTab === "inventory" ? "bg-white text-emerald-700 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}
          >
            <PackageSearch className="w-4 h-4 shrink-0" /> Network Inventory
          </button>
          <button 
            onClick={() => setActiveTab("audit")} 
            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-bold transition-all whitespace-nowrap shrink-0 ${activeTab === "audit" ? "bg-white text-emerald-700 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}
          >
            <History className="w-4 h-4 shrink-0" /> System Audit Logs
          </button>
          <button 
            onClick={() => setActiveTab("incoming")} 
            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-bold transition-all whitespace-nowrap shrink-0 ${activeTab === "incoming" ? "bg-white text-blue-700 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}
          >
            <Truck className="w-4 h-4 shrink-0" /> Incoming Transfers
            {incomingTransfers.length > 0 && (
              <span className="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full animate-pulse">{incomingTransfers.length}</span>
            )}
          </button>
          <button 
            onClick={() => setActiveTab("procurement")} 
            className={`flex items-center gap-2 px-4 py-1.5 rounded-md text-sm font-bold transition-all whitespace-nowrap shrink-0 ${activeTab === "procurement" ? "bg-white text-blue-700 shadow-sm" : "text-neutral-500 hover:text-neutral-700"}`}
          >
            <FilePlus className="w-4 h-4 shrink-0" /> Procurement Queue
          </button>
        </div>
      </div>

      {/* --- INVENTORY MODULE TAB CONTENT --- */}
      {activeTab === "inventory" && (
        <div className="space-y-4 pt-1">
          
          {/* THE NEW DEDICATED TOOLBAR ROW */}
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            
            {/* Left side: Site Dropdown Filter */}
            <div className="flex items-center bg-white border border-neutral-300 rounded-lg px-3 py-2 shadow-sm w-full lg:w-72 shrink-0">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setSiteFilter(val ? Number(val) : null);
                  setPivotSiteName(val ? sitesList.find(s => s.id === Number(val))?.site_name || "" : "");
                }}
                className="bg-transparent text-sm font-bold text-neutral-700 outline-none w-full cursor-pointer"
              >
                <option value="">All Network Sites</option>
                {sitesList.map(s => (
            ...
            <div className="flex flex-wrap items-center gap-2">
              {isDeleteMode ? (
                <>
                  <button onClick={() => { setIsDeleteMode(false); setSelectedIds([]); }} className="px-4 py-2 bg-white border border-neutral-200 text-neutral-700 hover:bg-neutral-100 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shrink-0">
                    <X className="w-4 h-4" /> Cancel Selection
                  </button>
                  <button onClick={handleBulkDelete} disabled={selectedIds.length === 0} className="px-4 py-2 bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shadow-sm shrink-0">
                    <Trash2 className="w-4 h-4" /> Delete Selected ({selectedIds.length})
                  </button>
                </>
              ) : (
                <>
                  <button onClick={() => { setIsDeleteMode(true); setShowAddForm(false); setShowBulkWizard(false); }} className="px-4 py-2 bg-white border border-red-200 text-red-600 hover:bg-red-50 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shrink-0">
                    <ListChecks className="w-4 h-4" /> Select Items
                  </button>
                  <button onClick={exportToCSV} className="px-4 py-2 bg-white border border-neutral-200 text-neutral-700 hover:bg-neutral-50 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shrink-0">
                    <Download className="w-4 h-4" /> Export
                  </button>
                  <button onClick={() => { setShowBulkWizard(!showBulkWizard); if (!showBulkWizard) setShowAddForm(false); }} className="px-4 py-2 bg-white border border-neutral-200 text-neutral-700 hover:bg-neutral-50 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shrink-0">
                    <Upload className="w-4 h-4" /> {showBulkWizard ? "Cancel Bulk Import" : "Bulk Import"}
                  </button>
                  <button onClick={() => { setShowAddForm(!showAddForm); if (!showAddForm) setShowBulkWizard(false); }} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shrink-0">
                    <BookmarkPlus className="w-4 h-4" /> {showAddForm ? "Cancel Setup" : "Register Item Baseline"}
                  </button>
                </>
              )}
            </div>
          ...
          {/* Existing Isolating Filter Banner */}
          {siteFilter && (
            <div className="bg-indigo-50 border border-indigo-200 p-4 rounded-xl flex items-center justify-between animate-in fade-in">
              <div className="flex items-center gap-2 text-indigo-800 font-medium text-sm">
                <Filter className="w-4 h-4" /> Isolating Inventory for Project Site:{" "}
                <strong className="font-black">{pivotSiteName || `SITE-${siteFilter}`}</strong>
              </div>
              <button onClick={() => { setSiteFilter(null); navigate(".", { replace: true, state: {} }); }} className="px-3 py-1 bg-white border border-indigo-200 text-indigo-600 hover:bg-indigo-100 rounded text-xs font-bold transition-colors">
          ...
          {showBulkWizard && (
            <BulkImportWizard sitesList={editableSites} suppliersList={suppliersList} onComplete={() => { setShowBulkWizard(false); fetchData(); window.dispatchEvent(new Event("inventoryUpdated")); }} onCancel={() => setShowBulkWizard(false)} />
          )}
          ...
          {showAddForm && (
            <div className="bg-white p-6 rounded-xl border border-emerald-200 shadow-sm animate-in slide-in-from-top-4">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 pb-4 border-b border-neutral-100">
                <div>
                  <h3 className="text-base font-bold text-neutral-900">Register Material Baseline</h3>
                  <p className="text-xs text-neutral-500">Set physical starting stock and alert thresholds for your site ledger.</p>
                </div>
              ...
              <form onSubmit={handleAddInventory} className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                ...
                <div>
                  ...
                  <input type="text" className="w-full p-2 border rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="e.g. Republic" value={newItem.brand} onChange={(e) => setNewItem({ ...newItem, brand: e.target.value })} />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <select className="w-full p-2 border rounded-lg text-sm bg-white focus:ring-2 focus:ring-emerald-500 outline-none font-medium" value={newItem.unit} onChange={(e) => setNewItem({ ...newItem, unit: e.target.value })}>
                      {itemType === "consumable" ? (
                        <><option value="Bags">Bags</option><option value="Pcs">Pcs</option><option value="Kilos">Kilos</option><option value="Linear Ft">Linear Ft</option><option value="Cu.m">Cu.m</option></>
                      ) : (
                        <><option value="Unit">Unit</option><option value="Set">Set</option></>
                      )}
                    </select>
                  </div>
                </div>
                <button type="submit" className="bg-slate-900 text-white py-2 rounded-lg font-bold hover:bg-slate-800 transition-colors w-full h-full max-h-[38px] flex items-center justify-center">
                  Register Baseline
                </button>
              </form>
            </div>
          ...
          <div className="flex gap-2 mb-4 overflow-x-auto pb-2 mt-4">
            ...
            {["All", "Critical", "Low Stock", "In Stock", "Surplus", "Sufficient", "Depleted"].map((f) => (
              <button key={f} onClick={() => setStatusFilter(f)} className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${statusFilter === f ? "bg-slate-800 text-white shadow-md" : "bg-white border border-neutral-200 text-neutral-600 hover:bg-neutral-50"}`}>
                {f}
              </button>
            ))}
          </div>
                          ...
                          <td className="px-5 py-4 w-48">
                            <div className="flex flex-col items-end gap-1">
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`text-base font-bold ${item.status === 'Depleted' ? 'text-neutral-400 line-through' : 'text-neutral-900'}`}>
                                  {item.quantity} <span className="text-neutral-400 font-normal text-xs">/ {item.baseline_quantity} {item.unit}</span>
                                </span>
                              </div>
                              {!isAsset && (
                                <div className="w-full bg-neutral-200 rounded-full h-1.5 mt-1 overflow-hidden">
                                  <div className={`h-1.5 rounded-full ${progressPercent <= 10 ? 'bg-red-500' : progressPercent <= 30 ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{ width: `${progressPercent}%` }}></div>
                                </div>
                              )}
                            </div>
                          </td>
                            ...
                            <div className="flex justify-end gap-1">
                              ...
                              {canEdit ? (
                                <>
                                  ...
                                  {/* PM ONLY: ERP 1-Click Request Restock */}
                                  {currentUserRole === "staff" && (
                                    <>
                                      {isCriticalOrLow && !isAsset && !isDeleteMode && (
                                        <button
                                          onClick={() => {
                                            setRequestItem(item);
                                            setRequestQty(Math.max(1, item.baseline_quantity - item.quantity));
                                            setShowRequestModal(true);
                                          }}
                                          className="p-2 ml-1 text-amber-600 hover:bg-amber-50 rounded-lg transition-colors flex items-center gap-1 text-xs font-bold border border-amber-200 shadow-sm"
                                          title="Send Replenishment Request to Admin"
                                        >
                                  ...
                                  {/* ADMIN ONLY: Free-Form Transfer & Advisor */}
                                  {currentUserRole !== "staff" && !isDeleteMode && (
                                    <>
                                      <button onClick={() => handleSmartRestock(item)} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors flex items-center gap-1 text-xs font-bold" title="Logistics Advisor">
        ...
        <div className="bg-white border border-neutral-200 rounded-xl shadow-sm overflow-hidden animate-in slide-in-from-bottom-4">
          <div className="px-6 py-5 border-b border-neutral-200 bg-blue-50 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-semibold text-blue-900">Pending Internal Deliveries</h2>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-neutral-50 text-neutral-500 border-b border-neutral-200">
                <tr>
                  ...
                  <th className="px-5 py-3 font-medium">Route (Origin → Destination)</th>
      ...
      {activeTab === "procurement" && (
        <div className="bg-white border border-neutral-200 rounded-xl shadow-sm overflow-hidden animate-in slide-in-from-bottom-4">
          <div className="px-6 py-5 border-b border-neutral-200 bg-blue-50 flex items-center justify-between">
            <div>
              <h2 className="font-semibold text-blue-900 flex items-center gap-2"><FilePlus className="w-5 h-5" /> Procurement Queue</h2>
              <p className="text-xs text-blue-700 mt-0.5">Track external orders from suppliers and confirm physical delivery into your inventory.</p>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-neutral-50 text-neutral-500 border-b border-neutral-200">
                <tr>
                  ...
                  <th className="px-6 py-3 font-medium text-center">Qty Ordered</th>
                  <th className="px-6 py-3 font-medium text-center">Status</th>
                  <th className="px-6 py-3 font-medium text-right">Fulfillment Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-neutral-100">
                    ...
                    return (
                      <tr key={po.id} className="hover:bg-neutral-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${po.status === "Pending" ? "bg-amber-100 text-amber-700 border border-amber-200" : po.status === "Shipped" || po.status === "In-Transit" ? "bg-blue-100 text-blue-700 border border-blue-200" : "bg-slate-100 text-slate-600 border border-slate-200"}`}>
                            {po.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            {po.status === "Pending" && (
                              <>
                                <button onClick={() => handleCancelPO(po.id)} className="px-3 py-1.5 border border-red-200 text-red-600 hover:bg-red-50 rounded-lg transition-colors text-xs font-bold inline-flex items-center gap-1.5 shadow-sm"><Ban className="w-3.5 h-3.5" /> Cancel</button>
                                <button disabled className="px-3 py-1.5 bg-neutral-200 text-neutral-400 rounded-lg cursor-not-allowed text-xs font-bold inline-flex items-center gap-1.5 shadow-sm"><Clock className="w-3.5 h-3.5" /> Awaiting Shipment</button>
                              </>
                            )}
                            {(po.status === "Shipped" || po.status === "In-Transit") && (
                              <button onClick={() => { setReceivingPO(po); setPoRating(0); }} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors text-xs font-bold inline-flex items-center gap-1.5 shadow-sm animate-pulse"><CheckCircle className="w-3.5 h-3.5" /> Confirm Delivery</button>
                            )}
                          </div>
                        </td>
                      </tr>
      ...
      {showRequestModal && requestItem && createPortal(
        <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-4 border-b bg-amber-50 border-amber-100 flex justify-between items-center text-amber-900">
              <h2 className="text-lg font-bold flex items-center gap-2">
                <ShoppingCart className="w-5 h-5 text-amber-600" /> Request Material Restock
              </h2>
              <button onClick={() => setShowRequestModal(false)} className="text-amber-700 hover:text-amber-900"><X className="w-5 h-5" /></button>
            </div>
            ...
            <form onSubmit={handleRequestRestock} className="p-6 bg-white space-y-5">
              <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-200">
                ...
                <p className="font-bold text-neutral-900">{requestItem.item_name} <span className="font-normal text-neutral-500">({requestItem.brand})</span></p>
                <p className="text-xs text-neutral-500 mt-1 font-medium">Site: {requestItem.siteName} • Current Stock: {requestItem.quantity} {requestItem.unit}</p>
              </div>

              <div>
                <label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Quantity Needed</label>
                <div className="flex items-center gap-3">
                  <input
                    type="number"
                    min="1"
                    ...
                    className="w-full p-3 border border-neutral-300 rounded-lg text-lg font-bold text-center focus:ring-2 focus:ring-amber-500 outline-none"
                  />
              ...
              <div className="pt-2">
                <button type="submit" className="w-full py-3 rounded-lg text-sm font-bold transition-colors text-white bg-amber-600 hover:bg-amber-700 flex justify-center items-center gap-2">
      ...
      {modalType === "LIFECYCLE" && activeTransactionItem && createPortal(
        <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-4 border-b flex justify-between items-center bg-indigo-50 text-indigo-900">
              <h3 className="font-bold flex items-center gap-2 text-lg"><Flag className="w-5 h-5 text-indigo-600" /> Manage Status Lifecycle</h3>
              <button onClick={() => setModalType(null)} className="text-indigo-400 hover:text-indigo-600"><X className="w-5 h-5"/></button>
            </div>
            <div className="p-5 space-y-4">
              <div className="bg-neutral-50 p-3 rounded-lg border border-neutral-200">
                <p className="text-xs text-neutral-500 font-bold uppercase mb-1">Target Material</p>
                <p className="font-bold text-neutral-900">{activeTransactionItem.item_name}</p>
                <p className="text-xs text-neutral-500 mt-1">Remaining: {activeTransactionItem.quantity} / {activeTransactionItem.baseline_quantity} {activeTransactionItem.unit}</p>
              </div>
              <div className="space-y-3 pt-2">
                ...
                <button onClick={() => handleOverrideStatus("Depleted")} className="w-full text-left p-4 rounded-xl border border-neutral-300 bg-white hover:bg-neutral-100 transition-colors group"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-neutral-200 flex items-center justify-center text-neutral-600 group-hover:scale-110 transition-transform"><Ban className="w-4 h-4"/></div><div><h4 className="font-bold text-neutral-900">Flag as "Depleted" (Used Up)</h4><p className="text-xs text-neutral-500 mt-0.5">Material is completely used up and no longer needed for this phase. Finalize lifecycle.</p></div></div></button>
              </div>
            </div>
          </div>
        </div>, document.body
