...
export function Inventory() {
  ...
  // FIXED: Moved inside the component so it can call fetchData()
  const handleDelete = async (id: number) => {
    if (
      !window.confirm(
        "Are you sure you want to remove this item from the ledger?",
      )
    )
      return;
    try {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">
            Manage stock across {sitesList.length} project sites.
          </p>
        </div>
          ...
          >
            ...
            <div>
              ...
              >
                ...
                <option value="In Transit">In Transit (Incoming)</option>
              </select>
            </div>
            <button
              type="submit"
              className="bg-emerald-600 text-white py-2 rounded-lg font-bold hover:bg-emerald-700"
            >
              Save Item
            </button>
          </form>
