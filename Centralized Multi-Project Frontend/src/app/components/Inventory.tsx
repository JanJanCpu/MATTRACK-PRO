...
export function Inventory() {
  ...
  const runAdvisorCalculation = async () => { if (!restockItem) return; setIsRestockLoading(true); setHasScanned(true); try { 
    // Using the live Render URL instead of localhost/hostname
  const response = await fetch(`https://mattrack-personal.onrender.com/advisory/auto-restock/${restockItem.site_id}?item_name=${encodeURIComponent(restockItem.item_name)}&quantity_needed=${restockQty}`); if (response.ok) setRestockOptions(await response.json()); } catch (error) { alert("Network error: Could not reach the heuristic engine."); } finally { setIsRestockLoading(false); } };
