...
#--- IMPORTS FOR FUEL PRICE SCRAPER ---
from apscheduler.schedulers.background import BackgroundScheduler
from pytz import timezone
from fuel_scraper import fetch_live_diesel_price
from database import SessionLocal # You need this for the manual DB session
import models
...
def calculate_transfer_cost(distance_km: float, quantity_needed: float, db: Session):
    # 1. Fetch the live fuel price from your database
    config = db.query(models.SystemConfiguration).filter(models.SystemConfiguration.config_key == "diesel_price_php").first()
    
    # 2. Use the live price, or fallback to 74.03 if the DB is empty
    FUEL_PRICE_PHP = float(config.config_value) if config else 74.03
    
    # 3. Calculate how many trucks are needed
    ...
    # ---> YOUR TURN <---
    # Calculate 'total_fuel_cost' by multiplying distance, fuel price, and trips
    # Calculate 'total_dispatch_fee' by multiplying the dispatch fee by trips
...
def start_scheduler():
    # 1. Set the timezone so "midnight" means Philippine Time, not Server Time
    scheduler = BackgroundScheduler(timezone=timezone('Asia/Manila'))
    
    # 2. Tell it to run your function every day at exactly 00:00 (midnight)
    scheduler.add_job(sync_fuel_to_db, 'cron', hour=0, minute=0)
    
    # 3. Start the engine
    scheduler.start()
    print("⏰ Background scheduler started! Fuel sync set for midnight.")
    
    # 4. (Optional but recommended) Run it once manually right now!
    # This guarantees your database has a price inside it immediately, 
    # instead of waiting until midnight tonight.
    sync_fuel_to_db()

# --- FUEL SCHEDULER ---
def sync_fuel_to_db():
    print("🔄 Running scheduled GasWatch fuel sync...")
    
    # 1. Fetch the live price from your scraper
    live_price = fetch_live_diesel_price()
    
    # 2. Open a manual database session
    db = SessionLocal()
    try:
        config = db.query(models.SystemConfiguration).filter(models.SystemConfiguration.config_key == "diesel_price_php").first()
        if config:
            config.config_value = live_price
        else:
            new_config = models.SystemConfiguration(config_key="diesel_price_php", config_value=live_price)
            db.add(new_config)
        db.commit()

        print(f"✅ Fuel price updated in DB: ₱{live_price}")
    except Exception as e:
        print(f"❌ Failed to sync fuel price: {e}")
        db.rollback()
    finally:
        # 3. ALWAYS close the manual session
        db.close()
...
def log_stock_transaction(transaction: schemas.InventoryBase = Body(...), current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    item = db.query(models.Inventory).filter(
        ...
        models.Inventory.item_name.ilike(transaction.item_name), 
        models.Inventory.brand.ilike(transaction.brand)
    ).first()
    ...
    if item:
        ...
        inv_data = transaction.dict(exclude={"supplier_id", "batch_rating", "is_locked_status"})
...
def get_smart_restock_options(site_id: int, item_name: str, quantity_needed: float, db: Session = Depends(get_db)):
    ...
    for item in surplus_items:
        ...
        est_cost = calculate_transfer_cost(dist, quantity_needed, db) 
