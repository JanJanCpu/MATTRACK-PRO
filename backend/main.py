...
# FIX: Added round constraint parameters to bypass passlib's internal initialization 72-byte test wrap bug
...
def create_access_token(data: dict):
    ...
    # SECURITY FIX: Swap out the deprecated .utcnow() function to maintain compatibility with Python 3.13+
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    # 1. Open the cryptographic token to find out who is asking
    try:
    ...
    # 2. Grab their full profile from the database
...
def get_managers(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    # Optional: You could add a check here to ensure only admin/owners can pull this list
    managers = db.query(models.User).filter(models.User.role == "staff").all()
    return managers
...
def list_sites(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    # Owner & Admin see everything across the entire company
    if user_role in ["owner", "admin"]:
        return db.query(models.ProjectSite).all()
    
    # Project Managers (staff) only see sites explicitly assigned to their user ID
...
def create_site(site: schemas.SiteCreate, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    # 1. Open the token to see who clicked "Create"
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        user_role: str = payload.get("role", "").lower()
        user_id: int = payload.get("id")
    ...
    # 2. Determine who gets assigned ownership
    assigned_manager = site.manager_id
    
    # If a Project Manager (staff) creates it, automatically bind it to their own User ID!
    if user_role == "staff":
        assigned_manager = user_id

    # 3. Save to database using the dynamic ID instead of 'None'
    db_site = models.ProjectSite(
        site_name=site.name, 
        latitude=site.lat, 
        longitude=site.lon,
        manager_id=assigned_manager # <--- The crucial fix is right here
    )
    db.add(db_site)
    db.commit()
    db.refresh(db_site)
...
def update_site_progress(site_id: int, req: schemas.SiteProgressUpdate, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    # Security Guard: Ensure PMs don't try to manipulate progress updates on other managers' sites
    if user_role == "staff" and site.manager_id != user_id:
        raise HTTPException(status_code=403, detail="Unauthorized write operation on this resource boundary")
...
def list_inventory(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    # Join the inventory query with project_sites to only pull items where the site manager matches the user ID
