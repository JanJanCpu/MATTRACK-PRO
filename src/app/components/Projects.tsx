...
import { sitesAPI, geocodeAddress } from "../../services/apiService";
...
export function Projects() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await sitesAPI.create({ name, lat: position[0], lon: position[1] });
      setName("");
      setAddress("");
                ...
                sites.map((site) => (
                  <tr key={site.id} className="hover:bg-neutral-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button onClick={() => navigate(`/projects/${site.id}`)} className="text-emerald-600 hover:text-emerald-700 hover:underline font-bold transition-colors">
