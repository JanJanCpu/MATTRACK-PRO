...
import { authAPI } from "../../services/apiService"; // Double check this path matches your file tree!
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        // Safe mapping using optional chaining fallbacks
        setUserName(data?.username || "User");
        setUserRole(data?.role || "Staff");
        setEmail(data?.email || "");
        setCompany(data?.company_name || "PENTABUILD Corp.");
      } catch (e) {
        console.error("Profile payload sync error:", e);
      }
    };
    loadProfile();
  }, []);
    ...
    <div className="space-y-6 animate-in fade-in duration-500 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-neutral-500 mt-1">
          Manage your account, roles, and company preferences.
        </p>
      </div>
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-white border border-neutral-200 rounded-xl p-6 shadow-sm text-center">
            ...
            <p className="text-xs text-neutral-500 mt-4 px-4">
              As an {userRole}, you have full access to inventory ledgers and
              procurement advisory tools.
            </p>
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    disabled
                    value={userName}
                    className="w-full p-2.5 bg-neutral-100 border border-transparent rounded-lg text-sm text-neutral-600 cursor-not-allowed"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-neutral-500 uppercase mb-1">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-white border border-neutral-300 rounded-lg text-sm outline-none focus:border-emerald-500"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-neutral-500 uppercase mb-1">
                ...
                <input
                  defaultValue="Makati City, Metro Manila"
                  className="w-full p-2.5 bg-white border border-neutral-300 rounded-lg text-sm outline-none focus:border-emerald-500"
                />
              </div>
