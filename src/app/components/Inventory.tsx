...
export function Inventory() {
  ...
  // ---> PASTE THE AUTO-SELECT HOOK RIGHT HERE <---
  useEffect(() => {
    if (sitesList.length === 1 && !newItem.site_id) {
      setNewItem((prev) => ({ ...prev, site_id: String(sitesList[0].id) }));
    }
  }, [sitesList, newItem.site_id]);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      alert(
        "An error occurred while deleting some items. Please check the console.",
      );
      console.error(err);
      fetchData();
    }
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `MatTrack_Inventory_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const filteredData =
    filter === "All"
      ...
      : inventoryData.filter(
          (i) => i.category === filter || i.status === filter,
        );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">
            Manage stock and track audit logs across {sitesList.length} project
            sites.
          </p>
        </div>
            ...
            <tbody className="divide-y divide-neutral-100">
              {filteredData.length > 0 ? (
                filteredData.map((item) => (
                  <tr
                    key={item.id}
                    className={`hover:bg-neutral-50/50 transition-colors ${selectedIds.includes(item.id) ? "bg-red-50/30" : ""}`}
                  >
